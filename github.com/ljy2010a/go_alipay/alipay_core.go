...
func buildRequestMysign(kv *Kvpairs, alipayConfig *AlipayConfig) string {
	...
	case "RSA2":
		prestr := createLinkstringUrlencode(kv)
		mysign, err = rsa2Sign(prestr, alipayConfig.Private_key)
		if err != nil {
			mysign = "rsaSign err"
		}
		break
 ...
 * return 签名结果字符串
 */
func buildRequestMysignForPost(kv *Kvpairs, alipayConfig *AlipayConfig) string {
	mysign := ""
	var err error
	beego.Debug(alipayConfig.Sign_type)
	switch alipayConfig.Sign_type {
	case "RSA":
		prestr := createLinkstringForPost(kv)
		beego.Debug(prestr)
		mysign, err = rsaSign(prestr, alipayConfig.Private_key)
		if err != nil {
			mysign = "rsaSign err"
 ...
 * @return 要请求的参数数组
 */
func buildRequestParaForPost(kv *Kvpairs, alipayConfig *AlipayConfig) {
	//除去待签名参数数组中的空值和签名参数
	paraFilter(kv)
	...
	//生成签名结果
	mysign := buildRequestMysignForPost(kv, alipayConfig)
...
func verifySign(u url.Values, alipayConfig *AlipayConfig) error {
	...
	for k := range u {
		...
		case "sign":
			...
			continue
		case "sign_type":
			if u.Get("service") != "alipay.service.check" {
				continue
			}
		}
	...
	case "RSA2":
		return rsa2Verify(prestr, encodeString, alipayConfig.Public_key)
	}

...
func AlipayWebRequestForm(alipayConfig *AlipayConfig, r *AlipayWebRequest, w io.Writer) error {
	p := Kvpairs{
		...
		Kvpair{`seller_id`, alipayConfig.Partner},
	...
	paraFilter(&p)
	argSort(&p)
	//sign := RsaSign(&p, alipayConfig)
	//beego.Debug("rsa sign:", sign)
	...
	// 坑，用post中文有问题
	fmt.Fprintf(w, `<form name='alipaysubmit' action='%s' method='get'> `, alipayGatewayNew)
	for _, kv := range p {
